...
@router.post("/forgot-password", status_code=status.HTTP_202_ACCEPTED)                  # we always return 202 whether email exists or not
async def forgot_password(
    ...
    if user:
        ...
        background_tasks.add_task(                                  # scheduling the email to be sent in the background
            send_password_reset_email,                              # function that we add to its task, and below are its parameters
            to_email=user.email,
            username=user.username,
            token=token,
        )
...
async def reset_password(
    ...
    await db.execute(
        sql_delete(models.PasswordResetToken).where(                                            # deletes all reset tokens for this user
            models.PasswordResetToken.user_id == user.id,
        ),
    )
