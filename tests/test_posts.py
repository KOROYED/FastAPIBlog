...
@pytest.mark.anyio                                                      # this is the decorator that enbables running async test funcs
async def test_get_posts_empty(client: AsyncClient):
    response = await client.get("/api/posts")                           # our configured async client works WITHOUT IMPORTING coz of pytest fixture injection
                                                                        # When pytest runs a test it looks at test's parameters and tries to match each param name to -
                                                                        # a fixture with the same name, it searches the cur test file first and then conftest.py in -
                                                                        # the same directory and then any conftest.py files in parent dirs.
...
async def test_create_post_success(client: AsyncClient):
    ...
    headers = auth_header(token)                                                        # this three are authentication setup, helper functions from conftest are used
...
async def test_get_posts_with_pagination(client: AsyncClient):
    ...
    for i in range(5):
        response = await client.post(
            "/api/posts",
            json={"title": f"Post {i}", "content": "Content for post {i}"},
            headers=headers,
        )
        assert response.status_code == 201
